...
bridgePol.events.Transfer(
  ...
  .on('data', async event => {
    try {
      ...
      const tx = bridgeEth.methods.mint(adminEthereum, userAddress, amount, newAmount, nonce, signature);
      const [gasPrice, gasCost] = await Promise.all([
        ethereumWebSocket.eth.getGasPrice(),
        tx.estimateGas({ from: adminEthereum }),
      ]);
      ...
      const receipt = await ethereumWebSocket.eth.sendTransaction(txData);
